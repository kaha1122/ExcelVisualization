  ...
  return (
    <Paper
      ...
      sx={{
        ...
        '&:hover': { transform: 'translateY(-8px)', background: 'rgba(30, 41, 59, 0.8)', borderColor: color, boxShadow: `0 8px 32px ${color}33` },
      }}
    >
      <input type="file" hidden ref={inputRef} accept=".xlsx, .xls" onChange={(e) => e.target.files?.[0] && onFileSelect(e.target.files[0])} />
    ...
    <Box sx={{ minHeight: '100vh', py: 8 }}>
      <Container maxWidth="lg">
        <Box sx={{ textAlign: 'center', mb: 10 }}>
          <Typography variant="h2" gutterBottom sx={{ fontWeight: 800, background: 'linear-gradient(45deg, #6366f1, #ec4899)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>{t('title')}</Typography>
          <Typography variant="h5" color="text.secondary" sx={{ maxWidth: 700, mx: 'auto' }}>{t('description')}</Typography>
        </Box>
          ...
          <Fade in={true}>
            <Box sx={{ textAlign: 'center', py: 6 }}>
              <Paper sx={{ p: 6, background: 'rgba(16, 185, 129, 0.05)', border: '1px solid #10b981' }}>
                ...
                <Typography variant="body2" color="text.secondary" sx={{ mb: 4 }}>AI 에이전트들이 협업하여 도출한 고도화된 보고서를 지금 바로 다운로드하세요.</Typography>
                <Stack direction="row" sx={{ display: 'flex', justifyContent: 'center', gap: 3 }}>
                  <Button variant="contained" size="large" color="primary" startIcon={<TableChartIcon />} onClick={() => generateExcelReport(agentResults)} sx={{ px: 4 }}>Excel 보고서 다운로드</Button>
                  <Button variant="contained" size="large" color="secondary" startIcon={<SlideshowIcon />} onClick={() => generatePPTReport(agentResults)} sx={{ px: 4 }}>PPT 보고서 다운로드</Button>
                </Stack>
              </Paper>
            </Box>
          </Fade>
        ...
        {!workflow.activeAgentId && (
          <Stack direction="row" sx={{ mt: 4, display: 'flex', justifyContent: 'center', gap: 2 }}>
            <Button variant="contained" size="large" onClick={startWorkflow} disabled={!files.master || !files.analyze} sx={{ px: 6, py: 1.5, fontSize: '1.1rem', background: 'linear-gradient(45deg, #4f46e5, #6366f1)' }}>{t('start_analysis')}</Button>
          </Stack>
        )}
      </Container>
    </Box>
